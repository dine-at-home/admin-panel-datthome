...
export default function UserDetailPage() {
  ...
  const fetchUserDetails = async () => {
    try {
      ...
      const response = await fetch(getApiUrl(`/admin/users/${userId}/details`), { headers })
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            <div className="flex items-center space-x-2">
              ...
              {user.blocked && (
                <Ban className="h-5 w-5 text-red-500" title="Blocked" />
              )}
            </div>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
          ...
          {user.languages && (
            <div>
              ...
              <span className="ml-2 text-sm text-gray-900">{user.languages}</span>
            </div>
          )}
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              ...
              {user.accountHolderName && (
                <div>
                  ...
                  <span className="ml-2 text-gray-900">{user.accountHolderName}</span>
                </div>
              )}
              {user.iban && (
                <div>
        ...
        <div className="bg-white shadow rounded-lg p-4">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{statistics.totalDinners}</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-8 px-6 overflow-x-auto" aria-label="Tabs">
            {[
              ...
              { id: 'bookings-guest', label: `Bookings as Guest (${data.bookingsAsGuest.length})` },
              { id: 'bookings-host', label: `Bookings as Host (${data.bookingsAsHost.length})` },
              { id: 'reviews', label: `Reviews (${data.reviewsWritten.length + data.reviewsReceived.length})` },
          ...
          {activeTab === 'dinners' && (
            <div>
              {data.dinners.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No dinners created</p>
              ) : (
          ...
          {activeTab === 'bookings-guest' && (
            <div>
              {data.bookingsAsGuest.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No bookings as guest</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {data.bookingsAsGuest.map((booking) => (
                        <tr key={booking.id}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {booking.dinner.host.name || booking.dinner.host.email}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
          ...
          {activeTab === 'bookings-host' && (
            <div>
              {data.bookingsAsHost.length === 0 ? (
                <p className="text-gray-500 text-center py-8">No bookings as host</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
            ...
            <div className="space-y-6">
              <div>
                ...
                {data.reviewsWritten.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">No reviews written</p>
                ) : (
                  <div className="space-y-4">
                    {data.reviewsWritten.map((review) => (
                      <div key={review.id} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium text-gray-900">{review.dinner.title}</span>
                          <div className="flex items-center">
              ...
              <div>
                ...
                {data.reviewsReceived.length === 0 ? (
                  <p className="text-gray-500 text-center py-4">No reviews received</p>
                ) : (
                  <div className="space-y-4">
                    {data.reviewsReceived.map((review) => (
                      <div key={review.id} className="border rounded-lg p-4">
                        ...
                        <p className="text-sm text-gray-600 mb-1">{review.dinner.title}</p>
                        {review.comment && (
                          <p className="text-sm text-gray-600">{review.comment}</p>
                        )}
                        <p className="text-xs text-gray-400 mt-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {data.favorites.map((favorite) => (
                    <div key={favorite.id} className="border rounded-lg overflow-hidden">
                      ...
                      <div className="p-4">
                        <h4 className="font-medium text-gray-900">{favorite.dinner.title}</h4>
